...
class GDELTdata(sparkSession: SparkSession, input: String, inputCameo: String) {

  /*sql scheme of GDELT dataset*/
  var CameoSchema = StructType(Array(
  ...
  var Cameo = sparkSession.read
    .option("header", "true")
    .option("delimiter", "\t")
    .option("nullValue", "")
    .option("treatEmptyValuesAsNulls", "true")
    .schema(this.CameoSchema)
    .csv(inputCameo)
  ...
  def Actors1: DataFrame = {
    val gdelt_reduced = gdelt
      .select("GLOBALEVENTID", "EventCode", "Actor1Code", "Actor1Name", "Actor1Type1Code", "Actor1Type2Code", "Actor1Type3Code", "Actor1KnownGroupCode")
      .where("Actor1Code is not null")
    return gdelt_reduced
  }


  def readEvents: DataFrame = {

    val gdelt_reduced = gdelt
      .select("GLOBALEVENTID", "EventCode")
    return gdelt_reduced
  }
}